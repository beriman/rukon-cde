...
export class AuthService {
    ...
    try {
      ...
      if (!data.email) {
        throw new UnauthorizedException(
          "Invalid Google Token: Email claim missing",
        );
      }
      return data.email;
    } catch (error) {
  ...
  async googleSync(dto: GoogleSyncDto) {
    // Verify Google Token
    const googleEmail = await this.verifyGoogleToken(dto.accessToken);

    if (googleEmail !== dto.email) {
      throw new UnauthorizedException(
        "Email mismatch between Token and Request",
      );
    }
