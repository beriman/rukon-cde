...
export class FilesService {
    ...
    async uploadSystemFile(
        ...
        // SENTINEL: Sanitize inputs to prevent path traversal
        const safeOrgId = path.basename(organizationId);
        const safeSubfolder = path.basename(subfolder);
        const safeFilename = path.basename(file.originalname);

        const s3Key = `org-${safeOrgId}/system/${safeSubfolder}/${timestamp}-${safeFilename}`;
        ...
        if (process.env.AWS_S3_BUCKET) {
            ...
            // Local fallback
            const fs = require('fs');
            // path is imported globally now
            const uploadDir = path.join(process.cwd(), 'uploads', `org-${safeOrgId}`, 'system', safeSubfolder);
