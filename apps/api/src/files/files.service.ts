...
export class FilesService {
  ...
  async upload(folderId: string, file: any, uploadedBy: string) {
    ...
    if (folder.permissions && folder.permissions["write"]) {
      ...
      if (!allowedRoles.includes("*")) {
        ...
        const hasPermission = allowedRoles.some(
          (r) => r === `role:${userRole}` || r === `user:${uploadedBy}`,
        );

        if (!hasPermission) {
          throw new BadRequestException(
            "You do not have permission to upload to this folder (Read Only).",
          );
        }
      }
    }
      ...
      if (existingFile) {
        ...
        if (file.originalname.toLowerCase().endsWith(".rvt")) {
          this.conversionService
            .processFile(updatedFile.id, s3Key)
            .catch(console.error);
        }
    ...
    // Audit Log
    this.auditService
      .log(uploadedBy, AuditAction.FILE_UPLOAD, result.id, "FILE", {
  ...
  async generateDownloadUrl(id: string, version?: number) {
    ...
    if (!targetVersion) {
      throw new NotFoundException(
        `Version ${version || file.currentVersion} not found`,
      );
    }

    const stubUrl = `https://s3.amazonaws.com/${process.env.AWS_S3_BUCKET || "rukon-cde"}/${targetVersion.s3Key}`;
    console.log(
      `[DOWNLOAD STUB] Would generate presigned URL for: ${targetVersion.s3Key}`,
    );
  ...
  async archive(id: string, userId: string) {
    ...
    await this.auditService
      .log(userId, AuditAction.FILE_DELETE, id, "FILE", {
  ...
  async uploadSystemFile(
    ...
    // Validation for subfolder to prevent path traversal
