...
export class FilesService {
    ...
    async uploadSystemFile(
        ...
        // Sentinel: Security enhancement to prevent path traversal
        const safeOriginalName = path.basename(file.originalname).replace(/[^a-zA-Z0-9.\-_]/g, '_');
        // Sanitize subfolder to prevent directory traversal up
        const safeSubfolder = subfolder.replace(/\\/g, '/').replace(/\0/g, '').replace(/(^|\/)\.\.(?=\/|$)/g, '').replace(/^\/+/, '');
