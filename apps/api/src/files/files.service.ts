...
export class FilesService {
    ...
    async uploadSystemFile(
        ...
        const safeSystemOriginalName = path.basename(file.originalname).replace(/[^a-zA-Z0-9.\-_]/g, '_');
        const s3Key = `org-${organizationId}/system/${subfolder}/${timestamp}-${safeSystemOriginalName}`;
        ...
        if (process.env.AWS_S3_BUCKET) {
            ...
            const filePath = path.join(uploadDir, `${timestamp}-${safeSystemOriginalName}`);
            fs.writeFileSync(filePath, file.buffer);
        }
