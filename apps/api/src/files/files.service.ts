...
export class FilesService {
    ...
    async uploadSystemFile(
        ...
        // Sanitize originalname and subfolder to prevent path traversal
        const safeOriginalName = path.basename(file.originalname.replace(/\\/g, '/'));
        let safeSubfolder = subfolder.replace(/\\/g, '/').replace(/(^|\/)\.\.(?=\/|$)/g, '');
        safeSubfolder = safeSubfolder.replace(/^\/+/, ''); // remove leading slashes
