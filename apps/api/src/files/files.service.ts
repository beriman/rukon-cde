...
export class FilesService {
    ...
    async upload(
        ...
        const safeFilenameOriginal = path.basename(file.originalname);

        // Story 1.14: Validate ISO 19650 naming convention
        const validation = this.namingService.validate(safeFilenameOriginal);

        if (!validation.isValid) {
            throw new BadRequestException({
                message: 'Invalid file naming convention',
                error: validation.error,
                yourFilename: safeFilenameOriginal,
            });
        }
        ...
        const result = await this.prisma.$transaction(async (tx) => {
            ...
            if (existingFile) {
                ...
                if (safeFilename.toLowerCase().endsWith('.rvt')) {
        ...
        const safeFilenameForAudit = path.basename(file.originalname);
