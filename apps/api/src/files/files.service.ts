...
export class FilesService {
    ...
    async uploadSystemFile(
        ...
        // Prevent path traversal on subfolder string input by explicitly rejecting bad characters
        if (subfolder && (subfolder.includes('..') || subfolder.includes('\0'))) {
            throw new BadRequestException('Invalid subfolder path');
        }

        const safeSubfolder = subfolder ? subfolder.replace(/\\/g, '/').replace(/^\/+/, '') : '';
        const folderPathPart = safeSubfolder ? `${safeSubfolder}/` : '';
        ...
        const s3Key = `org-${organizationId}/system/${folderPathPart}${timestamp}-${safeOriginalName}`;
