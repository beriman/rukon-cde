...
import {
  CreateSiteCaptureDto,
  UpdateSiteCaptureDto,
  QuerySiteCaptureDto,
} from "./site-capture.dto";
...
export class SiteCaptureService {
  ...
  async getProjectStats(projectId: string) {
    const [total, photos, videos, voiceNotes, unsynced] = await Promise.all([
      ...
      this.prisma.siteCapture.count({
        where: { projectId, captureType: "PHOTO" },
      }),
      this.prisma.siteCapture.count({
        where: { projectId, captureType: "VIDEO" },
      }),
      this.prisma.siteCapture.count({
        where: { projectId, captureType: "VOICE_NOTE" },
      }),
